        ...
        return np.array(
            [
                (
                    ...
                    float(raw_vector[i * 3 + 1]),
                    float(raw_vector[i * 3 + 2]),
                )
                for i in H36CompatibleJoints.joint_idx
            ]
        )
...
class MPII3DDataset(pose_dataset_base.PoseDatasetBase):
    def __init__(
        self,
        annotations_glob="/mnt/dataset/MPII_INF_3DHP/mpi_inf_3dhp/*/*/annot.mat",
        train=True,
    ):
        self.dataset = []
        for annotation_path in glob.glob(annotations_glob):
            ...
            for camera in MPII3DDatasetUtil.mm3d_chest_cameras:
                for frame in range(len(annotation["annot2"][camera][0])):
                    annot_2d = H36CompatibleJoints.convert_points(
                        annotation["annot2"][camera][0][frame]
                    )
                    annot_3d = H36CompatibleJoints.convert_points_3d(
                        annotation["annot3"][camera][0][frame]
                    )
                    ...
                    self.dataset.append(
                        MPII3DDatum(
                            ...
                            normalized_annotation_2d=self._normalize_2d(
                                annot_2d.reshape(
                                    -1, len(H36CompatibleJoints.joint_idx) * 2
                                )
                            ),
    ...
    def get_example(self, i):
        return (
            self.dataset[i].normalized_annotation_2d,

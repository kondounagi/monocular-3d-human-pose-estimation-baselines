...
from dataset.pose_dataset import Normalization
...
class H36M(Dataset):
    def __init__(self, action="all", length=1, train=True, use_sh_detection=False):
        ...
        if not os.path.exists("data/h36m/points_3d.pkl"):
            ...
            os.system(
                'wget --no-check-certificate "https://onedriv'
        ...
        for s in subjects:
            for action_name in actions:
                ...
                def search(a):
                    fs = list(filter(lambda x: x.split()[0] == a, p3d[s].keys()))
                    return fs
    ...
    def __getitem__(self, i):
        ...
        X, scale = Normalization.normalize_3d(X)
        X = X.astype(np.float32)
        scale = scale.astype(np.float32)

        if self.use_sh_detection:
            # StackedHourglassにより検出されたPoseの正規化
            sh_detect_xy = Normalization.normalize_2d(sh_detect_xy)

...
def load_model(pth_path: str):
    # NOTE: pytorch lightning save hparams file below
    hparams_file = os.path.join(os.path.dirname(pth_path), "..", "hparams.yaml")
    model = PoseNet.load_from_checkpoint(pth_path, hparams_file=hparams_file)
    model.eval()
    return model
